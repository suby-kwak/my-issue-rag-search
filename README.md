...
GitHub Issues ─┐
               │ (n8n: Webhook)
Notion 메모  ──┼──────────────► n8n (자동화 허브) ──► POST /api/ingest ──► pgvector
               │                                                            │
               └──── 카카오톡/슬랙(예정) ◄── POST /api/query ◄── 질문 ◄─────┘
```

- **n8n**이 GitHub/Notion 이벤트 감지, 메신저 연동 등 모든 외부 연동을 전담합니다.
- 이 Spring Boot 앱은 GitHub/Notion/Slack/Kakao API를 직접 호출하지 않고, `POST /api/ingest`·`POST /api/query` 두 개의 HTTP 엔드포인트로만 n8n과 통신합니다.
